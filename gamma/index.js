...
$("#stat").html(
"["+Math.round(speed / 6)+"/"+Math.round(abs_speed / 6)+"]words/min"+
"&nbsp;&nbsp;&nbsp;&nbsp;"+
"["+Math.round(speed)+"/"+Math.round(abs_speed)+"]chars/min"+
"&nbsp;&nbsp;&nbsp;&nbsp;"+
...
klava.text = data.replace(/\t/g, " ").replace(/ {2,}/g, " ").replace(/^ /g, "").replace(/\r/g, "").replace(/\n\n/g,"\n").split("\n");
...
is_not_error = true;
$("#text" + (i + 1)).html('<span class="printed">'+convertToHtml(klava.lines[i].text.substring(0, input.length))+'</span>'+convertToHtml(klava.lines[i].text.substring(input.length)));
} else {
$("#text" + (i + 1)).html(convertToHtml(klava.lines[i].text));
}
}
